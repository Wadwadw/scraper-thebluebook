...
class V3Spider(scrapy.Spider):
    ...
    def __init__(self):
        self.user_list = []
        with open(r'C:\Games\trainee\pr1\project1\project1\spiders\user.txt', 'r') as file:
            for line in file:
    ...
    def parse(self, response):
        ...
        if 'Captcha' in driver.title:
            ...
            driver.execute_script(f"document.getElementsByClassName('g-recaptcha-response')[0].innerHTML = '{resp}';")
            WebDriverWait(driver, 3).until(lambda d: d.find_element_by_xpath("//div[contains(@class,'single_result_wrapper')]/div[@class='media']/div[contains(@class,'media-body')]"))
        ...
        for normal in normales:
            yield {
                ...
                'City': normal.xpath(
                    ".//div[@class='addy_wrapper']/span/span[@itemprop='addressLocality']/text()").get(),
                'State': normal.xpath(
                    ".//div[@class='addy_wrapper']/span/span[@itemprop='addressRegion']/text()").get(),
                'Zip-code': normal.xpath(
                    ".//div[@class='addy_wrapper']/span/span[@itemprop='postalCode']/text()").get(),
                'Phone-number': normal.xpath(".//span[@itemprop='telephone']/text()").get(),
                'Website': normal.xpath(".//a[@class='website-link external-trigger']/@href").get(),
                'Page': self.page
            }

        next_page = respons.xpath("(//div[@class='pager-outer-wrapper']/div)[last()]/a/@href").get()
        if next_page:
